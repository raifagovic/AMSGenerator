...
if let config = readConfigFile() {
    var (name, address, identificationNumber, payerName, payerAddress, payerCountry) = config
    
    // Override with values provided via command-line flags
    let commandLineValues = parseCommandLineArgs()
    if !commandLineValues.name.isEmpty {
        name = commandLineValues.name
    }
    if !commandLineValues.address.isEmpty {
        address = commandLineValues.address
    }
    if !commandLineValues.identificationNumber.isEmpty {
        identificationNumber = commandLineValues.identificationNumber
    }
    if !commandLineValues.payerName.isEmpty {
        payerName = commandLineValues.payerName
    }
    if !commandLineValues.payerAddress.isEmpty {
        payerAddress = commandLineValues.payerAddress
    }
    if !commandLineValues.payerCountry.isEmpty {
        payerCountry = commandLineValues.payerCountry
    }
